...
export default new Vuex.Store({
  ...
  mutations: {
    INITIALIZE_STORE (state) {
      state.khulnasoftApi 
        = localStorage.getItem('khulnasoftApi')
      state.timeseriesApi 
    ...
    SET_KHULNASOFT_API (state, khulnasoftApi) {
      state.khulnasoftApi = khulnasoftApi
      localStorage.setItem('khulnasoftApi', khulnasoftApi)
    },
  ...
  actions: {
    ...
    async saveSettings ({commit}, settings) {
      await commit('SET_KHULNASOFT_API', settings.khulnasoftApi)
    ...
    async fetchAccessToken ({commit, state}) {
      ...
      let result = await axios.post(
          state.khulnasoftApi + '/api/v1/login', 
          loginSettings
        )
    ...
    async fetchNamespaces ({commit, state}) {
      ...
      let response = await axios.get(state.khulnasoftApi + '/api/v1/orchestrator/namespaces/names?orderby=name%2Basc', 
    ...
    async fetchRegistries ({commit, state}) {
      ...
      let response = await axios.get(state.khulnasoftApi + '/api/v1/registries', 
        { headers: { Authorization: tokenString } 
      })
    ...
    async fetchImages ({commit, state}, repo) {
      ...
      if (repo !== 'All+Registries') {
        response = await axios.get(state.khulnasoftApi + '/api/v2/images?registry=' + repo + '&page=1&include_totals=true&order_by=name&page_size=1000', 
          { headers: { Authorization: tokenString } 
        })
      } else {
        response = await axios.get(state.khulnasoftApi + '/api/v2/images?page=1&include_totals=true&order_by=name&page_size=1000', 
          { headers: { Authorization: tokenString } 
        })
      }
    ...
    async fetchAllRisks ({commit, state}) {
      ...
      for (var i = 0; i < pageLoop; i++) {
        vulnResponse = await axios.get(
          state.khulnasoftApi + '/api/v2/risks/vulnerabilities',
        { 
          headers: { 
    ...
    async fetchRisks ({commit, state}) {
      //https://testdrive656.khulnasoft.com/api/v2/images/khulnasoftdemo/malware-example/latest/malware
      //https://testdrive656.khulnasoft.com/api/v2/images/khulnasoftdemo/malware-example/latest/sensitive
      await commit('SET_RISKS', [])
      await commit('SET_RISK_TABLE_LOADING', true)
      let tokenString = "Bearer " + state.accessToken
      let vulnResponse = await axios.get(state.khulnasoftApi + '/api/v2/risks/vulnerabilities', 
        { 
          headers: { 
            Authorization: tokenString 
          },
      ...
      for (let i = 0; i < vulnResponse.data.result.length; i++) {
        if (vulnResponse.data.result[i].khulnasoft_severity !== 'medium') {
      ...
      let malwareResponse = await axios.get(`${state.khulnasoftApi}/api/v2/images/${state.repoSelected}/${cleansedImageName}/malware`, 
      { 
        headers: { 
          Authorization: tokenString 
        }
      ...
      let sensitiveResponse = await axios.get(`${state.khulnasoftApi}/api/v2/images/${state.repoSelected}/${cleansedImageName}/sensitive`, 
      { 
        headers: { 
          Authorization: tokenString 
        }
    ...
    async fetchVulnAck ({commit, state}) {
      ...
      /////////////////////////////////////
      let tokenString = "Bearer " + state.accessToken
      let response = await axios.get(state.khulnasoftApi + '/api/v2/risks/vulnerabilities?include_vpatch_info=true&show_negligible=true&page=1&pagesize=1000&skip_count=false&hide_base_image=false&acknowledge_status=true&order_by=-vendor_severity', 
        { headers: { Authorization: tokenString } 
      })
    ...
    async fetchVulnSeverity ({commit, state}, severity) {
      let tokenString = "Bearer " + state.accessToken
      let result = await axios.get(state.khulnasoftApi + '/api/v2/risks/vulnerabilities?include_vpatch_info=true&show_negligible=false&page=1&pagesize=50&skip_count=true&hide_base_image=false&severity=' + severity + '&order_by=-vendor_severity', 
        { headers: { Authorization: tokenString } 
      })
