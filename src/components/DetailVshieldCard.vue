...
<template v-slot:item.first_found_date="{ item }">
  <div
    v-if="item.khulnasoft_severity === 'critical'"
  >
    {{moment(item.first_found_date).add(criticalAge, 'days').fromNow()}}
  </div>
  <div
    v-if="item.khulnasoft_severity === 'high'"
  >
   ...
   computed: {
    ...
    critCount () {
      return this.risks.filter(function(obj) {
        return obj.khulnasoft_severity === 'critical'
      }).length
    },
    highCount () {
      return this.risks.filter(function(obj) {
        return obj.khulnasoft_severity === 'high'
      }).length
    },
    medCount () {
      return this.risks.filter(function(obj) {
        return obj.khulnasoft_severity === 'medium'
      }).length
    },
    criticalAge () {
